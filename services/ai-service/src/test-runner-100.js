...
async function runTests() {
  ...
  console.log(`🚀 Starting Comprehensive HTTP E2E Test Suite for 100 queries via ${endpoint}...`);
  ...
  for (const tc of testCases) {
    // Enforce a brief throttle delay to prevent TCP socket exhaustion on local fetch calls
  ...
  fs.writeFileSync(
    path.join(scriptDir, 'e2e-results-100.json'),
    JSON.stringify({ summary: { total: testCases.length, passed: passedCount, failed: failedCount }, results }, null, 2)
  );

  // Generate beautiful markdown report artifact
  const artifactDir = "C:\\Users\\Masfia Computer\\.gemini\\antigravity-ide\\brain\\3b760a38-6d0b-4741-a856-66100b1b0cba";
  const reportPath = path.join(artifactDir, 'test_suite_report.md');
  
  let mdContent = `# E2E AI Search Verification Report\n\n`;
  mdContent += `Generated on: ${new Date().toISOString()}\n\n`;
  mdContent += `### Summary Status\n`;
  mdContent += `- **Total Queries Tested**: ${testCases.length}\n`;
  mdContent += `- **Successfully Passed**: ${passedCount} ✅\n`;
  mdContent += `- **Failed**: ${failedCount} ${failedCount === 0 ? '🎉' : '❌'}\n`;
  mdContent += `- **Success Rate**: **${((passedCount / testCases.length) * 100).toFixed(2)}%**\n\n`;
  
  mdContent += `### Test Results Details\n\n`;
  mdContent += `| ID | Query | Expected IDs | Actual Matches | Status |\n`;
  mdContent += `|---|---|---|---|---|\n`;
  
  for (const r of results) {
    mdContent += `| ${r.id} | \`${r.query}\` | \`${JSON.stringify(r.expected)}\` | \`${JSON.stringify(r.actual)}\` | ${r.passed ? '✅ PASS' : '❌ FAIL'} |\n`;
  }
  
  fs.writeFileSync(reportPath, mdContent);
  console.log(`✍️ Beautiful E2E Test Suite Markdown Report written to ${reportPath}!`);
