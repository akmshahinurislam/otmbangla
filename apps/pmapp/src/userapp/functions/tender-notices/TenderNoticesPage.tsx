...
export function TenderNoticesPage() {
  ...
  const prevFiltersRef = useRef({ searchTerm, selectedCategories, selectedOrganizations, selectedLocations, dateFrom, dateTo });
  
  const filtersChanged = 
    prevFiltersRef.current.searchTerm !== searchTerm ||
    prevFiltersRef.current.selectedCategories !== selectedCategories ||
    prevFiltersRef.current.selectedOrganizations !== selectedOrganizations ||
    prevFiltersRef.current.selectedLocations !== selectedLocations ||
    prevFiltersRef.current.dateFrom !== dateFrom ||
    prevFiltersRef.current.dateTo !== dateTo;

  if (filtersChanged) {
    prevFiltersRef.current = { searchTerm, selectedCategories, selectedOrganizations, selectedLocations, dateFrom, dateTo };
    setPage(1);
    setHasMore(true);
  }
  ...
  useEffect(() => {
    ...
    async function fetchTenders() {
      try {
        ...
        const limit = page === 1 ? 25 : 20;
        const skip = page === 1 ? 0 : 25 + (page - 2) * 20;
        ...
        if (active) {
          if (page === 1) {
            ...
            setTenders(prev => {
              ...
              const filteredNewData = data.filter((t: TenderNotice) => !existingIds.has(t.id));
        ...
        if (active) {
          ...
          const paginatedSlice = result.slice(skip, skip + limit);
          if (page === 1) {
            setTenders(paginatedSlice);
          } else {
            setTenders(prev => {
              const existingIds = new Set(prev.map(t => t.id));
              const filteredNewData = paginatedSlice.filter((t: TenderNotice) => !existingIds.has(t.id));
              return [...prev, ...filteredNewData];
            });
          }
  ...
  }, [page, searchTerm, selectedCategories, selectedOrganizations, selectedLocations, dateFrom, dateTo]);
            ...
            <div ref={loaderRef} className="py-6 flex justify-center items-center">
              {isLoadingMore ? (
                <div className="flex items-center gap-2">
                  <div className="h-5 w-5 animate-spin rounded-full border-2 border-accent-purple border-t-transparent"></div>
                  <span className="text-xs text-muted dark:text-neutral-500 font-semibold">Loading more notices...</span>
                </div>
              ) : !hasMore && filteredTenders.length > 10 ? (
                <span className="text-xs text-muted dark:text-neutral-500 font-semibold">✨ You've reached the end of the list</span>
              ) : null}
            </div>
          </>
        ) : (
