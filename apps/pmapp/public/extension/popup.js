...
async function scanActiveTab() {
  try {
    ...
    try {
      ...
      if (!urlObj.hostname.endsWith("eprocure.gov.bd")) {
        showScanError("Restricted website.", "This AI Companion is configured to run exclusively on the Bangladesh National e-GP portal (**eprocure.gov.bd**). Please open an e-GP page to start scanning.");
        return;
      }
    } catch (e) {
      showScanError("Invalid URL.", "Could not parse the current tab URL.");
      return;
    }
